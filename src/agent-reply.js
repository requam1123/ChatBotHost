...
export async function runMockAgentReply(runID, agent, event, _unused) {
  ...
  const agentMessage = event.agentMessage || parseAgentMessagePayload(event);
  const runtimeEvent = agentMessage
    ? {
        ...event,
        agentMessage,
        content: agentPayloadToModelInput(agentMessage, event.content),
      }
    : event;

  const startTime = Date.now();
  const result = await buildAgentReplyForRuntime(agent, runtimeEvent, {
    runID,
    rootRunID: runID,
    parentRunID: '',
    depth: 0,
  ...
  let serverMsgID = '';
  let outputContent = replyText;
  let outputContentType = event.contentType;

  if (agentMessage?.kind === AgentMessageKind.Task) {
    const existingResultCall = findSuccessfulToolCall(runResult.toolCalls, 'send_agent_result');
    if (existingResultCall?.result?.serverMsgID) {
      serverMsgID = existingResultCall.result.serverMsgID;
      outputContent = existingResultCall.args?.result || replyText;
      outputContentType = AgentContentType.Result;
    } else {
      const resultPayload = buildAgentResultPayload({
        taskID: agentMessage.taskID,
        parentTaskID: agentMessage.parentTaskID,
        title: agentMessage.title,
        result: replyText,
        status: runResult.status === 'failed' ? 'failed' : 'success',
        sourceAgent: agent,
        replyToAgentUserID: agentMessage.sourceAgentUserID || event.sendID,
        requesterUserID: agentMessage.requesterUserID,
        metadata: {
          autoGenerated: true,
          runID,
          requestServerMsgID: event.serverMsgID || '',
          targetAgentUserIDs: agentMessage.targetAgentUserIDs || [],
          targetAgentNicknames: agentMessage.targetAgentNicknames || [],
        },
      });
      outputContent = formatAgentResultDisplay(resultPayload);
      outputContentType = AgentContentType.Result;
      const sent = await imClient.sendMessage({
        sendID: agent.imAgentUserID,
        recvID: event.groupID ? undefined : resultPayload.replyToAgentUserID || event.sendID,
        groupID: event.groupID || undefined,
        contentType: AgentContentType.Result,
        content: outputContent,
        ex: buildAgentMessageContent(resultPayload),
        atUserIDList: resultPayload.replyToAgentUserID ? [resultPayload.replyToAgentUserID] : [],
...
function findSuccessfulToolCall(toolCalls = [], toolID) {
  return (Array.isArray(toolCalls) ? toolCalls : []).find((call) =>
    call.toolID === toolID &&
    call.result &&
